...
def load_data_and_labels(filename="Dataset_Final.csv"):
    ...
    # Preprocessing teks
    x_text = [clean_str(sent) for sent in df['text'].tolist()]
    x_text = [s.split(" ") for s in x_text]
    
    # Mengubah label teks ke One-Hot Encoding
    # Contoh: 'Geography' -> [1, 0, 0], 'Biology' -> [0, 1, 0]
    y = pd.get_dummies(df['label']).values
    
    # Menyimpan daftar nama label untuk referensi nanti
    label_names = pd.get_dummies(df['label']).columns.tolist()
